...
import {TabtSubscription} from './subscription-model';
...
export interface MasterWorkerMapEntry {
    ...
    startTime: string;
    subscriptions: TabtSubscription[];  // Cluster worker process start time
}

@Service()
export class MasterWorkerCache {

    public _workersMap: Map<number, MasterWorkerMapEntry>;
    ...
    async registerWorkerAsync(clusterWorker: Worker): Promise<void> {
        ...
        const workerMapEntry: MasterWorkerMapEntry = {
            ...
            subscriptions: new Array<TabtSubscription>()
        };
    ...
    async getEntryBySybscriptionIdAsync(subscription: TabtSubscription): Promise<MasterWorkerMapEntry> {
        return new Promise((resolve, reject) => {
            try {
                for (const workerMapEntry of this._workersMap.values()) {
                    if (find(workerMapEntry.subscriptions, subscription)) {
