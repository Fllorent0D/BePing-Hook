    ...
    interval(intervalTime).pipe(
      ...
      .subscribe(async () => {
        ...
        for (const match of matchesToNotify) {
          try {
            await this.firebaseService.getDatabase().ref('/notifications/sent')
              .push(match.MatchId, (error: Error) => this.loggingService.error('Error when saving notification state', error));
            ...
            this.loggingService.error(`Error when saving notification flag for match ${match.MatchId}`, e);
          }
