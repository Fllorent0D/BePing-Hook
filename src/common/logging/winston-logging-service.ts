import {getMessage, LogEvent, LogEventPayload, LogLevel, LogLevels, LogMsgType, LogRecordConverter, LogTimer} from '.';
import {Cluster} from 'cluster';
import * as _ from 'lodash';
import {IEvent, IpcEventBus} from '../ipc-bus';
...
require('firebase-winston');

const winston = require('winston');
const format = winston.format;
const cluster: Cluster = require('cluster');

export type TransportTypes = 'Console' | 'File' | 'Firebase';
...
export class LoggingService {
    ...
    private static getLogLevelNbr(level: string): number {
        switch (level) {
            case LogLevels.DEBUG.label:
                return LogLevels.DEBUG.id;
            case LogLevels.INFO.label:
                return LogLevels.INFO.id;
            case LogLevels.WARN.label:
                return LogLevels.WARN.id;
            case LogLevels.ERROR.label:
                return LogLevels.ERROR.id;
    ...
    private createEnabledTransports(enabledTransports: any[]): any[] {
        const transports = enabledTransports
            .map((transportConfig: any) => this.createTransportFromConfig(transportConfig));
        return transports;
    }

    private createTransportFromConfig(transportConfig: any): void {
        switch (transportConfig.transport) {
            ...
            case 'Firebase':
                const firebaseOpts = {
                    ref: this.firebaseService.getDatabase().ref('/notifications/logging'),
                    level: transportConfig.minLevel
                };
                return new (winston.transports.FirebaseLogger)(firebaseOpts);
            default: {
                throw new Error(
    ...
    private createLoggerWithTransports(customLevels: any): void {
        ...
        const opts: any = {
            ...
            transports: this.createEnabledTransports(this._enableTransports),
            exitOnError: false
        };
